...
class HsImage:
    ...
    def __init__(self, dir_path, filename_pattern='-SPECTRAL_IMAGE', extensions=None):
        if extensions is None:
            extensions = ['TIF', 'tif', 'TIFF', 'tiff', 'BSQ', 'bsq']
        ...
        # test through different extensions
        for ext in extensions:
            self.image_path.extend(glob.glob(f"{dir_path}/*{filename_pattern}.{ext}"))

        if not self.image_path:
            raise FileNotFoundError(f"No files found with pattern '*{filename_pattern}'"
                                    f" and extensions {extensions}")
    ...
    def _load_image(self):
        """
        Load the hyperspectral image from file and set necessary attributes
        """
